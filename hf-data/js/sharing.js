...
async function createSharing() {
  ...
  function displayError(error) {
    ...
    if (error.id.includes('forbidden')) {
      message = `${
        error.message
      } Your token does not have enough permission to create a sharing. Try to reconnect.`;
    }
...
async function addListAccess(table, access) {
  // add permissions to the sharings table

  const permissions = [];
  for (const permission of access.permissions)
    permissions.push(permission.streamId);
  ...
  const emailSubject = encodeURIComponent(
    'Access my ' + permissions.join(', ') + ' data'
  );
  ...
  const deleteLink =
    '<a href="" onclick="javascript:deleteSharing(\'' +
    access.id +
    '\');return false;">' +
    access.name +
    '</a>';
